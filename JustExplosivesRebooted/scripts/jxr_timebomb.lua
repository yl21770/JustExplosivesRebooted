...
--[[function init(args)
  Use the code below for proximity bombs or anything that requires proximity detection
...
--[[function update(dt)
  if self.triggerTimer > 0 then
    ...
    if self.triggerTimer == 0 then
      --if config.getParameter("firstProjectile") ~= nil then
        world.spawnProjectile("timebombexplosion",
        entity.toAbsolutePosition({0.2, 1}), entity.id(), {0, 5}, false,{})
      --end
      object.setAllOutputNodes(true)
      object.smash()
    end
  else
    local radius = config.getParameter("detectArea")
    local entityIds = world.entityQuery(self.detectArea[1],
    self.detectArea[2], { includedTypes = { "creature" }, boundMode = "CollisionArea"})
    if #entityIds > 0 then
      trigger()
    end
  end]]